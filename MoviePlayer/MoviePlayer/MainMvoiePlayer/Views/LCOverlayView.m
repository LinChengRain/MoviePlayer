...
#import "LCSubtitleViewController.h"

@interface LCOverlayView () <LCSubtitleViewControllerDelegate>
...
@property (strong, nonatomic) LCSubtitleViewController *controller;
...
- (IBAction)showSubtitles:(id)sender {
    ...
    self.controller = [[LCSubtitleViewController alloc] initWithSubtitles:self.subtitles];
...
- (IBAction)toggleFilmstrip:(id)sender {
    [UIView animateWithDuration:0.35 animations:^{
        if (self.filmstripHidden) {
            ...
            self.filmStripView.y = 0;
        } else {
            self.filmStripView.y -= self.filmStripView.height;
        }
